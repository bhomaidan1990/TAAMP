...
from.meiying_collision_checking import object_collision
...
# Meiying project
YOUBOT_URDF = 'models/youbot.urdf'
...
# Meiying project objects mesh
SMALL_BLOCK_URDF = 'models/drake/objects/meiying_block_for_pick_and_place.urdf'
SHORT_SMALL_BLOCK_URDF = 'models/drake/objects/meiying_block_for_pick_and_place_short.urdf'
SHORT_HUGE_SMALL_BLOCK_URDF = 'models/drake/objects/meiying_block_for_pick_and_place_short_huge.urdf'
BLOCK_URDF = 'models/drake/objects/meiying_block_for_pick_and_place_mid_size.urdf'
CYAN_BLOCK_URDF = 'models/drake/objects/meiying_block_for_pick_and_place_mid_size_cyan.urdf'
SINK_URDF = 'models/meiying_sink.urdf'
STOVE_URDF = 'models/meiying_stove.urdf'
OCCLUDER_URDF = 'models/meiying_occluder.urdf'
DUCK_URDF = 'models/meiying_duck.urdf'
TOOL_TARGET_URDF = 'models/meiying_tool_target.urdf'
PULL_TOOL_URDF = 'models/meiying_L_pull_tool.urdf'
PULL_CHAIN_TOOL_URDF = 'models/meiying_L_chain_pull_tool.urdf'
PUSH_TOOL_URDF = 'models/meiying_T_push_tool.urdf'
TUNNEL_URDF = 'models/meiying_tunnel.urdf'
TUNNEL_CHAIN_URDF = 'models/meiying_tunnel_chain.urdf'
TUNNEL_CHAIN_AFFORDANCE_URDF = 'models/meiying_tunnel_chain_affordance.urdf'

PULL_TOOL_OLD_URDF = 'models/meiying_L_pull_tool_old.urdf'
PULL_CHAIN_TOOL_OLD_URDF = 'models/meiying_L_chain_pull_tool_old.urdf'
PUSH_TOOL_OLD_URDF = 'models/meiying_T_push_tool_old.urdf'

PLUNGER_TOOL_URDF = 'models/meiying_plunger.urdf'
SCREWDRIVER_TOOL_URDF = 'models/meiying_large_red_screwdriver.urdf'
...
#MAX_DISTANCE = 1e-3 / 2. # for n = 100000
MAX_DISTANCE = 1e-3 # for n = 10000, task 1
...
def get_closest_points(body1, body2, link1=None, link2=None, max_distance=MAX_DISTANCE, use_aabb=False, body_path={}):
...
def pairwise_link_collision(body1, link1, body2, link2=BASE_LINK, **kwargs):
    points = []
    closest_points = get_closest_points(body1, body2, link1=link1, link2=link2, **kwargs)
    if len(closest_points) != 0:
        collision_distances = []
        for point in closest_points:
            point_1 = np.array(point.positionOnA)
            point_2 = np.array(point.positionOnB)
            distance = np.linalg.norm(point_1 - point_2)
            if distance < MAX_DISTANCE:
                collision_distances.append(distance)
                points.append(point)
        #print("[Meiying::pairwise_link_collision] body {} and body {} collision distance:".format(body1, body2), collision_distances)        
    return len(points) != 0
...
def body_collision(body1, body2, body_path={}, **kwargs):
    num_points = 0       
    
    use_body_path = True
    if len(body_path.items()) == 0:
        use_body_path = False
    elif body1 not in body_path.keys() or body2 not in body_path.keys():
        use_body_path = False
    
    if use_body_path:
        #if (body1 == 4 and body2 == 5) or (body1 == 5 and body2 == 4):
            #raise Exception("checking 4 and 5 collision with my method...")
        #print("body1:", body1)
        #print("body2:", body2)
        #print("body_path keys:", body_path.keys())
        return object_collision(body1, body2, body_path=body_path)

    else:
        points = []
        closest_points = get_closest_points(body1, body2, **kwargs)
        if len(closest_points) != 0:
            collision_distances = []
            for point in closest_points:
                point_1 = np.array(point.positionOnA)
        ...
        #print("[Meiying::body_collision] body {} and body {} collision distance:".format(body1, body2), collision_distances)        
        
        
    #if body1 == 6 and body2 == 4:
        #print()
        #print()
        #print("occluder pose:", p.getBasePositionAndOrientation(body1)[0])
        #print("celery pose:", p.getBasePositionAndOrientation(body2)[0])
        #print("check result:", num_points != 0)
        #print()
        #print()      
    #elif body2 == 6 and body1 == 4:
        #print()
        #print()       
        #print("occluder pose:", p.getBasePositionAndOrientation(body2)[0])
        #print("celery pose:", p.getBasePositionAndOrientation(body1)[0])
        #print("check result:", num_points != 0)
        #print()
        #print()       
...
def pairwise_collision(body1, body2, body_path={}, **kwargs):
    if isinstance(body1, tuple) or isinstance(body2, tuple):
        body1, links1 = expand_links(body1)
        body2, links2 = expand_links(body2)
        #print("[Meiying::pairwise_collision] any_link_pair_collision({}, {}, {}, {}): {}".format(body1, links1, body2, links2, any_link_pair_collision(body1, links1, body2, links2, **kwargs)))
        return any_link_pair_collision(body1, links1, body2, links2, **kwargs)
    #print("[Meiying::pairwise_collision] body_collision({}, {}): {}".format(body1, body2, body_collision(body1, body2, **kwargs)))
    #if (body1 == 4 and body2 == 5) or (body1 == 5 or body2 == 4):
        #raise Exception("checking 4 and 5 collision...")    
    return body_collision(body1, body2, body_path=body_path, **kwargs)

def pairwise_collisions(body, obstacles, link=None, body_path={}, **kwargs):
    return any(pairwise_collision(body1=body, body2=other, link1=link, body_path=body_path, **kwargs)
...
def stable_z(body, surface, surface_link=None):
    return stable_z_on_aabb(body, get_aabb(surface, link=surface_link)) + 0.004
...
def sample_placement(top_body, bottom_body, bottom_link=None, **kwargs):
    ...
    pose = sample_placement_on_aabb(top_body, bottom_aabb, **kwargs)
    position = list(pose[0])
    position[2] += 0.004
    #return sample_placement_on_aabb(top_body, bottom_aabb, **kwargs)
    return (tuple(position), pose[1])
...
def grasp_from_body_ee(body_pose, ee_pose):
    """
    Pose_{gripper, body}
    """
    return multiply(invert(ee_pose), body_pose)
...
def save_command(current_directory, command):
    file_names = []
    for file_name in glob.glob(os.path.join(current_directory, "*.json")):
        current_file_name = os.path.splitext(os.path.basename(file_name))[0]
        try:
            current_file_name = int(current_file_name)
        except ValueError:
            current_file_name = None
        if not current_file_name is None:
            file_names.append(current_file_name)
    
    if len(file_names) == 0:
        file_name_index = 1
    else:
        file_name_index = max(file_names) + 1
    
    command_file_name = "{}.json".format(file_name_index)
    command_file_path = os.path.join(current_directory, command_file_name)
    
    with open(command_file_path, 'w') as json_file:
        json.dump(command.serialize(), json_file, indent = 4, sort_keys=True)           
...
#from learn_affordance_tamp import transformations as tfs
#from learn_affordance_tamp import kinematics as kt
#from learn_affordance_tamp import averageQuaternions as aq
#from learn_affordance_tamp import datastructure_util
#from learn_affordance_tamp import constants

import transformations as tfs
import kinematics as kt
import averageQuaternions as aq
import datastructure_util
import constants
...
def similar_translation(vector_1, vector_2, threshold=np.radians(5.0)):
    ...
    #print "angle: ", abs(get_angle([vector_1], [vector_2]))
...
#def similar_pose(pose_1, pose_2, screw_angle_error = 5.0, screw_distance_error = 0.01, screw_v_error = 5.0, theta_error = 5.0):
    #S_1, theta_1 = get_exponential_from_transformation_matrix(pose_1)
    #S_2, theta_2 = get_exponential_from_transformation_matrix(pose_2)
    
    #print "in similar pose!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
    
    #if theta_1 < np.radians(1.0) and theta_2 < np.radians(1.0):
        #print "small angle"
        #return True    
    
    #if not same_screw_axis(S_1, S_2, angle_error = screw_angle_error, distance_error = screw_distance_error, v_error = screw_v_error):
        #print "different screw axis"
        #return False
    
    #if abs(theta_1 - theta_2) > np.radians(theta_error):
        #print "different theta"
        #return False
    
    #return True

#def similar_pose(pose_1, pose_2, translation_threshold=np.radians(5.0), axis_threshold=0.005, point_threshold=0.005):
    ...
    #position = similar_translation(position_1, position_2, translation_threshold)
    #rotation = similar_rotation(quaternion_1, quaternion_2, axis_threshold, point_threshold)
    
    #print "position is similar: ", position
    #print "quaternion is similar: ", rotation
...
def same_rotation_helper(rotation_1, rotation_2):
    ...
    #print "alpha: ", abs(alpha_1 - alpha_2)
    #print "beta: ", abs(beta_1 - beta_2)
    #print "gamma: ", abs(gamma_1 - gamma_2)
...
def get_angle(start_vector, end_vector):
    #print "start vector" [1, 1, 1]
    #print start_vector

    #print "end vector" [1, 1, 1]
    #print end_vector
    
    #print "vector 1 length: ", get_vector_length(start_vector)
    #print "vector 2 length: ", get_vector_length(end_vector)
    #print "multiply length: ", (get_vector_length(start_vector) * get_vector_length(end_vector))
    #print "dot product: ", np.dot(start_vector[0], end_vector[0])
    #print "result: ", np.dot(start_vector[0], end_vector[0]) / (get_vector_length(start_vector) * get_vector_length(end_vector))
...
def get_rotation_matrix(axis, angle):
    ...
    #print("part 1")
    #print(np.cos(angle) * np.identity(3))
    #print("part 2")
    #print((1 - np.cos(angle)) * np.matmul(np.array([[axis[0][0]], [axis[0][1]], [axis[0][2]]]), np.array([[axis[0][0], axis[0][1], axis[0][2]]])))
    #print("part 3")
    #print(np.sin(angle) * kt.get_skew_symmetric_matrix(axis))
...
def average_screw_axis(screw_axis_list):
    ...
    #w = np.array([averaged[0], averaged[1], averaged[2]])
...
def _get_delta_with_chosen_screw_axis_from_transformation(direction, interval, transformation, S):
    ...
    while distance > 0.005 and i < num_interation:
        ...
        #print "{}: delta = {}, distance = {}, current_p = {}, final_p = {}, final_p - current_p = {}".format(i, np.rad2deg(delta), distance, current_p, final_p, final_p - current_p)
...
def get_delta_with_chosen_screw_axis_from_transformation(transformation, new_screw_axis):
    ...
    #old_screw_axis, old_theta = get_exponential_from_transformation_matrix(transformation)
    
    #return get_delta_with_different_screw_axis(old_screw_axis, old_theta, new_screw_axis)
...
def category_trajectory(trajectory): # trajectory is a list of transformations
    ...
    for i in range(1, len(screw_axes)):
        ...
        is_same_direction, is_same_screw_axis = same_screw_axis_with_same_direction(current_averaged_screw_axis, current_screw_axis, sum(current_thetas), current_theta, angle_error = 5.0, v_error = 20.0)
...
def is_scale_uniform_distribution(array):
    ...
    if num_cluster == 1:
        ...
        # The D value: the maximum absolute difference between the sample and the desired distribution (cdf)
        #if not is_uniform:
            #is_normal = kstest(array_for_kstest, 'norm', args=(means[0], covariances[0])).pvalue > 0.9
            #print "kstest normal result: ", kstest(array_for_kstest, 'norm', args=(means[0], covariances[0])).pvalue
            #if not is_normal:
                #is_uniform = True
...
def cluster_array_DBSCAN(array, eps=0.05, min_samples=2):
    print "[cluster_array_DBSCAN] array to cluster: "
    print array
    
    cluster = DBSCAN(eps=eps, min_samples=min_samples).fit(array)
    #label = cluster.labels_
    #noise = np.where(label == -1)[0]
    #label_without_noise = np.delete(label, noise)
...
def get_trajectory_interpolation_with_transformations(Tstart, Tend):
    ...
    #print "[transformation_util][get_trajectory_interpolation_with_transformations] Tstart"
    #print Tstart
    #print "[transformation_util][get_trajectory_interpolation_with_transformations] Tend"
    #print Tend    
...
def get_trajectory_interpolation_with_fixed_frame_screw_axis(S, theta, Tstart):
    #print "[transformation_util][get_trajectory_interpolation_with_fixed_frame_screw_axis] S: ", S
    #print "[transformation_util][get_trajectory_interpolation_with_fixed_frame_screw_axis] theta: ", theta
    #print "[transformation_util][get_trajectory_interpolation_with_fixed_frame_screw_axis] Tstart"
    #print Tstart    
    
    ## TODO: delete this chunk
    #num_interval = 2
    #interval = theta / (num_interval * 1.)
    
    #print "[transformation_util][get_trajectory_interpolation_with_fixed_frame_screw_axis] theta: ", theta
    
    # TODO: uncomment this chunck
    interval = theta
    num_interval = 1
    ...
    if is_screw_axis_translational(S):
        ...
        #interval = np.deg2rad(5.) # knock
        interval = np.deg2rad(30.) # stir
        num_interval = abs(int(theta / interval))

    interval *= direction
    #print "[transformation_util][get_trajectory_interpolation_with_fixed_frame_screw_axis] inverval: ", [interval * i for i in range(num_interval)]
    #print "[transformation_util][get_trajectory_interpolation_with_fixed_frame_screw_axis] Ss"
    ...
    #print "[transformation_util][get_trajectory_interpolation_with_fixed_frame_screw_axis] trajectory to return:"
    #for T in trajectory:
        #print T
...
def get_trajectory_interpolation_with_body_frame_screw_axis(S, theta, Tstart):
    #print "[transformation_util][get_trajectory_interpolation_with_body_frame_screw_axis] S: ", S
    #print "[transformation_util][get_trajectory_interpolation_with_body_frame_screw_axis] theta in degree: ", np.rad2deg(theta)
    #print "[transformation_util][get_trajectory_interpolation_with_body_frame_screw_axis] Tstart"
    #print Tstart
    ...
    #print "[transformation_util][get_trajectory_interpolation_with_body_frame_screw_axis] theta: ", theta    
    ...
    if is_screw_axis_translational(S):
        ...
        #interval = np.deg2rad(1.)
        interval = np.deg2rad(30.) # stir. draw
        #interval = np.deg2rad(5.) # knock
    ...
    #print "[transformation_util][get_trajectory_interpolation_with_body_frame_screw_axis] trajectory"
    #for T in trajectory:
        #print T

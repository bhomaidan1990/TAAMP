...
#def read_variable(file_path, name, variable_type, variable_collection_type=None):
    #variable_file = open(file_path, "r")
    #variable = None
    #variable_num = None
    #if variable_collection_type == TYPE_LIST:
        #variable = []
    ...
    #for line in variable_file:
        ...
        #elif found:
            #if variable_collection_type is None:
                #variable = convert_variable(line, variable_type)
                #break
            #elif variable_collection_type == TYPE_LIST:
                #if variable_num is None: # get the num of elements in the list
                    #variable_num = str_to_int(line)
                #else:
                    #if len(variable) < variable_num:
                        #variable.append(convert_variable(line, variable_type))
                    #else:
                        #break
            #elif variable_collection_type == TYPE_NESTED_LIST:
                #if variable_num is None: # get the num of elements in the list
                    #variable_num = str_to_int(line)
                    #variable = [[] for i in range(variable_num)]
                #else:
                    #if element_num is None: # get the num of variables in the element in the list
                        #element_num = str_to_int(line)
                    #else:
                        #if len(variable[current_index]) < element_num:
                            #variable[current_index].append(convert_variable(line, variable_type))
                        #else:
                            #if current_index + 1 >= len(variable):
                                #break
                            #else:
                                #element_num = str_to_int(line)
                                #current_index += 1

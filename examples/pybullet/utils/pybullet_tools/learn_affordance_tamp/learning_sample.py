...
from learn_affordance_tamp.constants import PERCEPTION_METHOD_ARUCO, PERCEPTION_METHOD_POINTCLOUD
from learn_affordance_tamp.constants import TASK_TYPE_POSE_CHANGE, TASK_TYPE_OTHER
from learn_affordance_tamp.constants import get_package_dir, get_perception_method
...
class Sample:
    ...
    def get_goalend(self):
        ...
        if self.task_type == TASK_TYPE_POSE_CHANGE:
            ...
            #print "in learning sample: "
            #print "self.Tworld_toolstart"
            #print self.Tworld_toolstart
            #print "self.Tworld_goalstart"
            #print self.Tworld_goalstart
            #print "----------------------------------------------"            
            #self.Ttool_goal = np.matmul(transformation_util.get_transformation_matrix_inverse(self.Tworld_toolstart), self.Tworld_goalstart)
        
        #print "self.Ttool_goal: "
        #print self.Ttool_goal
        ...
        #print self.trajectory
        #print "!!!!!!!!!!!!!!!!!!!!!!!!self.trajectory_screw_axes"
        #print self.trajectory_screw_axes
        #print "!!!!!!!!!!!!!!!!!!!!!!!!self.trajectory_thetas"
        #print self.trajectory_thetas
        ...
        if len(self.trajectory_screw_axes) != 0:
            #print "!!!!!!!!!!!!!!!!!!! PIKAPIKAPIKAPIKAPIKA NOT ZERO!!!!!!!!!!!!!!!!!!!!"
            ...
            ## get v_start of the goal change in the goal_start frame
            #Tgoalchange_worldframe = transformation_util.get_fixed_frame_transformation(self.Tworld_goalstart, self.Tworld_goalend)
            #S_world, theta_world = transformation_util.get_exponential_from_transformation_matrix(Tgoalchange_worldframe)
            #v_world = np.array([S_world[3], S_world[4], S_world[5]])
            #self.v_start_world_frame = transformation_util.normalize(v_world)              
            
            ## get v_start in the goal_start frame
            #Tgoalchange_goalframe = transformation_util.get_body_frame_transformation(self.Tworld_goalstart, self.Tworld_goalend)
            #S_goal, theta_goal = transformation_util.get_exponential_from_transformation_matrix(Tgoalchange_goalframe)
            #v_goal = np.array([S_goal[3], S_goal[4], S_goal[5]])
            #self.v_start_goal_frame = transformation_util.normalize(v_goal)        
            
            ## get v_start in the tool_start frame
            #Ttool_goalstart = self.Ttool_goal.copy()
            #Tgoalstart_goalend = np.matmul(transformation_util.get_transformation_matrix_inverse(self.Tworld_goalstart), self.Tworld_goalend)
            #Ttool_goalend = np.matmul(Ttool_goalstart, Tgoalstart_goalend)
            #Tgoalchange_toolframe = transformation_util.get_fixed_frame_transformation(Ttool_goalstart, Ttool_goalend)
            #S_tool, theta_tool = transformation_util.get_exponential_from_transformation_matrix(Tgoalchange_toolframe)
            #v_tool = np.array([S_tool[3], S_tool[4], S_tool[5]])
            #self.v_start_tool_frame = transformation_util.normalize(v_tool)
            
            #print "&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&"
            #print "S_world_frame: ",  S_world_frame
            #print "S_world: ", S_world
            #print "S_goal_frame: ", S_goal_frame
            #print "S_goal: ", S_goal
            #print "S_tool_frame: ", S_tool_frame
            #print "S_tool: ", S_tool

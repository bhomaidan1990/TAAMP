...
from .utils import get_pose, set_pose, get_movable_joints, \
    ...
    body_from_end_effector, get_model_path, BB_INFO

from .kuka_primitives import Command, BodyPose, BodyConf, BodyPath, Attach, assign_fluent_state, DEBUG_FAILURE, get_stable_gen, \
     _get_tool_use_config, _get_straightline_path, Detach, _get_tool_use_poses, stable_z
...
def _transform_bb(bb, transformation):
    ...
    ##current_rotation = bb.R
    
    #copied_bb.translate(np.array([0., 0., 0.]), relative=False)
    ##copied_bb.rotate(current_rotation.T) # for those, transpose is inverse
    
    #position, rotation = _decompose_homogeneous_transformation_matrix_to_rotation_matrix(transformation)
    
    #copied_bb.rotate(rotation)
    #copied_bb.translate(current_center)
    #copied_bb.translate(position)
    
    #return copied_bb

#def _is_obj_collide(body1, body1_bbs, body2, body2_bbs, body_path):
    #for body1_bb in body1_bbs:
        #for body2_bb in body2_bbs:
            #if _is_bb_collide(body1_bb, body2_bb):
                #return pairwise_collision(body1, body2, body_path=body_path)
...
def _is_obj_straightline_traj_feasible(body, pose, end_position, obstacles, body_path, exclude=[]):
    ...
    for position in positions:
        ...
        #print "[Meiying::meiying_primitives::_is_obj_straightline_traj_feasible] body pose:", pose.pose
        if _is_obj_env_collide(body, obstacles, body_bbs, intermediate_body_pose.pose, obstacles_bbs, body_path):
        #if any(_is_obj_collide(obstacle, body) for obstacle in obstacles):
            is_feasible = False
            break
    
    #print "[Meiying::meiying_primitives::_is_obj_straightline_traj_feasible] time:", end - start
    #print "[Meiying::meiying_primitives::_is_obj_straightline_traj_feasible] is_feasible:", is_feasible
    """
    
    return is_feasible 

#def _is_aabb_in_bound(body1_vertex, body2_min_bound, body2_max_bound):
    #body2_extent = body2_max_bound - body2_min_bound
    
    #for i in range(len(body1_min_bound)):
        #if body1_min_bound[i] < body2_min_bound[i] - abs(0.05 * body2_extent[i]):
            #return False
        #if body1_min_bound[i] > body2_max_bound[i] + abs(0.05 * body2_extent[i]):
            #return False
...
def _group_info(name, pcd, pose, bb):
    info = {}
    #info[name] = {}
    #info[name][POINTCLOUD] = pcd
    #info[name][POSE] = pose
    #info[name][BOUNDINGBOX] = bb
    info[POINTCLOUD] = pcd
    info[POSE] = pose
    info[BOUNDINGBOX] = bb    
...
#def get_place_object_traj_gen():
    #import inspect
    #def fn(o, p, fluents=[]):
        #print("[Meiying::get_place_object_traj_gen] PIKA sampling place motion!!!")
        ##print("stacks:")
        ##for line in inspect.stack():
            ##print(line)
        #object_pose = p.pose
...
#def get_place_ik_fn(robot, fixed=[], teleport=False, num_attempts=10):
    ##movable_joints = get_movable_joints(robot)
    ##sample_fn = get_sample_fn(robot, movable_joints)
    #def fn(body, pose, grasp, body_traj):
        #print("[Meiying::get_place_object_motion_gen] plan place ik!!!")
        ##obstacles = [body] + fixed
        ##gripper_pose = end_effector_from_body(pose.pose, grasp.grasp_pose)
        ##approach_pose = approach_from_grasp(grasp.approach_pose, gripper_pose)
        ##for _ in range(num_attempts):
...
def _find_place_area(body, surface, obstacles, body_path):
    ...
    while num_samples > 0:
        ...
        if not _is_obj_env_collide(body, obstacles, body_bbs, body_pose, obstacles_bbs, body_path):
            ...
            #for obstacle in obstacles:
                #if obstacle != FLOOR:
                    #obstacle_pcds.append(_get_pcd(obstacle, body_path))
            #to_visualize = obstacle_pcds
            #to_visualize.append(body_pcd)
            #o3d.visualization.draw_geometries(to_visualize, "finding place area, randomly sampling, this one is NOT colliding!!")
            
            #for obstacle_pcd in obstacle_pcds:
                #distance = np.asarray(body_pcd.compute_point_cloud_distance(obstacle_pcd))
                #min_distance = min(distance)
                #o3d.visualization.draw_geometries([body_pcd, obstacle_pcd], "min_distance = {}".format(min_distance))
    ...
    if not get_results:
        ...
        for x in x_choices:
            for y in y_choices:
                ...
                current_current_pose.assign()
                if not _is_obj_env_collide(body, obstacles, body_bbs, body_pose, obstacles_bbs, body_path):
                    #print "find a place to put the pose {} via brute force search.".format(body_pose)
                    #to_visualize = []
                    #for bbs in obstacles_bbs:
                        #to_visualize.extend(bbs)
                    ##to_visualize.extend(updated_body_bbs)
                    #to_visualize.append(_get_pcd(body, body_path))
                    #o3d.visualization.draw_geometries(to_visualize, "WORKS!!") 
...
def _find_pull_positions(body, pose, tool, obstacles, body_path, contact_pose=np.identity(4), manipulanda_tool_pose={}):
    ...
    if not check_tool_body_collision:
        ...
        while num_samples > 0:
            ...
            tool_start_pose, tool_end_pose = _get_tool_use_poses(body, tool, pose.pose, body_end_pose, contact_pose=contact_pose, manipulanda_tool_pose=manipulanda_tool_pose)
            ...
            #if not _is_obj_env_collide(tool, obstacles, tool_bbs, tool_start_pose, obstacles_bbs, body_path, visualize=False):
                #print "no collision!!"
                #num_non_colliding_examples += 1
                #if num_non_colliding_examples > 5:
                    #get_results = True
                    #is_pullable = True
                    #all_direction_feasible = True
                    #break
    ...
    if not all_direction_feasible:
        ...
        thetas = [i * 1.0 for i in range(360)]
        #thetas = [0., 180.]
        ...
        for theta in thetas:
            ...
            #print "find a contact pose that works!!"
            ...
            if not _is_obj_env_collide(tool, obstacles, tool_bbs, tool_start_pose, obstacles_bbs, body_path):
                ...
                for i in range(50):
                    ...
                    tool_start_pose, tool_end_pose = _get_tool_use_poses(body, tool, pose.pose, body_end_pose, contact_pose=contact_pose)
                    ...
                    if is_pcd_collide(obstacles_pcd, _merge_pcds([body_pcd, tool_pcd])):
                        ...
                        break
                    #for each_obstacle_bb in obstacles_bbs:
                        #if _is_bb_collide(current_tool_bb, each_obstacle_bb) or _is_bb_collide(current_body_bb, each_obstacle_bb):
...
Tests
"""
def get_pick_infeasible_test(fixed=[], body_path={}):
    def test(body, pose):
        print "[Meiying::get_pick_INfeasible_test]"
        obstacles = fixed
        end_position = (pose.pose[0][0], pose.pose[0][1], pose.pose[0][2] + 0.1)
        return not _is_obj_straightline_traj_feasible(body, pose, end_position, obstacles, body_path)
    return test

def get_pick_feasible_test(grouping, surface=[], fixed=[], body_path={}):
    def test(body, pose, *args):
...
#def get_place_connection_feasible_test(grouping, fixed=[], body_path=[]):
    #def test(body, region1, region2, *args):
        #return False
    
    #return test

def get_pull_feasible_test(grouping, fixed=[], body_path={}, contact_poses={}, pull_file_path=None, manipulanda_tool_pose={}):
    def test(body, tool, pose):
        ...
        is_pullable, all_direction_feasible, feasible_positions = _find_pull_positions(body, pose, tool, obstacles, body_path, contact_pose=contact_pose, manipulanda_tool_pose=manipulanda_tool_pose)
...
def get_place_object_gen(grouping, fixed=[], movables=[], body_path={}, file_path=None):
    def gen(body, surface, at_pose_fluents=[], *args):
        ...
        #print "[Meiying::meiying_primitives::get_place_object_gen] at_pose_fluents:", at_pose_fluents
        
        #if body == 8:
            #raise Exception("sample body 8 for place on surface {} with the other movable {}".format(surface, current_movables[0]))                
        print "Meiying::get_place_object_gen body {} on surface {}".format(body, surface)
        print "Meiying::get_place_object_gen fixed: ", fixed
        
        #if body == 13 and surface == fixed[0]:
            #yield (BodyPose(body, ((0., -0.5, 0.105), (0., 0., 0., 1.))), )
            #return
        #elif body == 4 and surface == 2:
            #yield (BodyPose(body, ((-0.5, 0., 0.155), (0., 0., 0., 1.))), )
            #return
...
def get_tool_motion_gen(robot, fixed=[], teleport=False, num_attempts=10, body_path={}, contact_poses={}, manipulanda_tool_pose={}, tool_put_down_z={}):
    ...
    #def fn(*args):
        ...
        #body_start_transformation = _get_homogeneous_transformation_matrix_from_quaternion(np.array(body_pose1.pose[1]), np.array(body_pose1.pose[0]))
        #body_start_pcd = _get_pcd(body, body_path, transformation=body_start_transformation)
        #body_start_pcd.paint_uniform_color((0., 1., 0.))
        #body_end_transformation = _get_homogeneous_transformation_matrix_from_quaternion(np.array(body_pose2.pose[1]), np.array(body_pose2.pose[0]))
        #body_end_pcd = _get_pcd(body, body_path, transformation=body_end_transformation)
        #body_end_pcd.paint_uniform_color((0., 0., 1.))
        
        #tool_start_transformation = _get_homogeneous_transformation_matrix_from_quaternion(np.array(tool_pose1[1]), np.array(tool_pose1[0]))
        #tool_start_pcd = _get_pcd(tool, body_path, transformation=tool_start_transformation)
        #tool_start_pcd.paint_uniform_color((0., 1., 0.))
        #tool_end_transformation = _get_homogeneous_transformation_matrix_from_quaternion(np.array(tool_pose2[1]), np.array(tool_pose2[0]))
        #tool_end_pcd = _get_pcd(tool, body_path, transformation=tool_end_transformation)
        #tool_end_pcd.paint_uniform_color((0., 0., 1.))
        
        #o3d.visualization.draw_geometries([body_start_pcd, body_end_pcd, tool_start_pcd, tool_end_pcd])
        
        #obstacles = [body, tool] + fixed
        #obstacles = fixed
        #obstacles = [body, tool] + fixed
        ...
        #tool_put_down_transformation = _get_homogeneous_transformation_matrix_from_quaternion(np.array(tool_put_down_pose[1]), np.array(tool_put_down_pose[0]))
        #tool_put_down_pcd = _get_pcd(tool, body_path, transformation=tool_put_down_transformation)
        #tool_put_down_pcd.paint_uniform_color((1., 0., 0.))
        
        #o3d.visualization.draw_geometries([body_start_pcd, body_end_pcd, tool_start_pcd, tool_end_pcd, tool_put_down_pcd])        
...
def get_pull_tool_goal_gen(body_pose_path, grouping, body_path, pull_file_path=None, fixed=[], movable=[], manipulanda_tool_pose={}):
...
def get_push_tool_goal_gen(body_pose_path, grouping, body_path, pull_file_path=None, fixed=[], movable=[], manipulanda_tool_pose={}):
    def gen(body, tool, surface, at_pose_fluents=[], *args): 
        # find body pose
        previous_body_poses = {}
        try:
...
#def get_body_sequence(movables=[]):
    #def fn():
        #for body in range(len(movables)):
            #obstacles = []
            #for obstacle in movables:
                #if obstacle != body:
                    #obstacles.append(obstacle)
            #obstacles.sort()
            #yield tuple(obstacles)
        ##yield (movable[0], movable[0], movable[0], movable[0])
    #return fn

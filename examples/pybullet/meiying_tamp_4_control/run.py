...
from examples.pybullet.utils.pybullet_tools.utils import WorldSaver, connect, dump_body, get_pose, set_pose, Pose, \
    ...
    BLOCK_URDF, SMALL_BLOCK_URDF, TOOL_TARGET_URDF, PULL_TOOL_URDF, PUSH_TOOL_URDF, get_configuration, SINK_URDF, STOVE_URDF, load_model, is_placement, get_body_name, \
...
def pddlstream_from_problem(robot, surface, body_path, tool_grasp_pose={}, contact_poses={}, movable=[], pull_tool=None, push_tool=None, teleport=False, grasp_name='top'):
    ...
    stream_map = {
        ...
        'plan-pull-ik': from_fn(get_tool_motion_gen(robot, fixed=fixed, body_path=body_path, contact_poses=contact_poses)),
        ...
        'sample-push-tool-goal': from_gen_fn(get_push_tool_goal_gen(fixed, movable)),
        'plan-push-ik': from_fn(get_tool_motion_gen(robot, fixed=fixed, body_path=body_path, contact_poses=contact_poses)),
...
def load_world():
    ...
    return robot, body_names, movable_bodies, body_path, pull_tool, push_tool, contact_poses, surface, tool_grasp_pose
...
def main():
    ...
    robot, names, movable, body_path, pull_tool, push_tool, contact_poses, surface, tool_grasp_pose = load_world()
    print('Objects:', names)
    saver = WorldSaver()

    problem = pddlstream_from_problem(robot, surface, body_path, tool_grasp_pose=tool_grasp_pose, contact_poses=contact_poses, movable=movable, pull_tool=pull_tool, push_tool=push_tool, teleport=args.teleport)

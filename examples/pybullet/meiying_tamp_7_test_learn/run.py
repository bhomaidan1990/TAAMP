...
from examples.pybullet.utils.pybullet_tools.meiying_primitives_affordance import get_pick_feasible_test, get_ik_fn, \
     get_holding_motion_gen, get_place_feasible_test, get_place_object_gen, get_move_feasible_test, get_grasp_gen, get_generic_feasible_test, group_initial_bodies
...
def pddlstream_from_problem(robot, surface, body_path, grouping, movable=[], teleport=False, grasp_name='top'):
    ...
    stream_map = {
        ...
        #'test-pick-infeasible': from_test(get_pick_infeasible_test(fixed, body_path)),
        'test-pick-infeasible': negate_test(get_generic_feasible_test(grouping, task="pick", surface=surface, fixed=fixed, body_path=body_path)),
        'test-place-feasible': from_test(get_generic_feasible_test(grouping, task="place", surface=surface, fixed=fixed, body_path=body_path)),
...
def load_world():
    ...
    with HideOutput():
        ...
        set_pose(occluder, Pose(Point(y=0.5, z=stable_z(occluder, floor))))        
    ...
    initial_index_grouping =[[sink], [stove], [celery, occluder], [radish]]
...
def main():
    ...
    problem = pddlstream_from_problem(robot, surface, body_path, grouping, movable=movable, teleport=args.teleport)

...
from examples.pybullet.utils.pybullet_tools.meiying_primitives_affordance import get_pick_feasible_test, get_ik_fn, get_generic_feasible_test, \
     ...
     get_tool_motion_gen, get_pull_tool_goal_gen, get_tool_use_generic_feasible_test

# delete these imports
# from examples.pybullet.utils.pybullet_tools.meiying_primitives import get_pull_feasible_test
#from examples.pybullet.utils.pybullet_tools.kuka_primitives import get_grasp_gen
...
def pddlstream_from_problem(robot, body_path, grouping, pull_tool=None, push_tool=None, movable=[], teleport=False, grasp_name='top'):
    ...
    stream_map = {
        ...
        'test-place-feasible': from_test(get_generic_feasible_test(grouping, task="place", surface=fixed, fixed=fixed, body_path=body_path, file_path=os.path.abspath(place_file_path))), 
        ...
        'sample-tool-goal': from_gen_fn(get_pull_tool_goal_gen(body_pose_path, grouping, body_path, fixed, movable)),
        'plan-pull-ik': from_fn(get_tool_motion_gen(robot, task="pull", fixed=fixed, body_path=body_path)),
        'test-pull-feasible': from_test(get_tool_use_generic_feasible_test(grouping, surface, task="pull", fixed=fixed, body_path=body_path, file_path=pull_file_path)),
...
def load_world():
    ...
    initial_index_grouping =[[sink], [stove], [celery], [radish], [pull_tool]]
    ...
    #tool_grasp_pose = {pull_tool: ((0.19, 0.1, 0.), (0., 0., 0., 1.))}    

    return robot, body_names, movable_bodies, body_path, grouping, pull_tool, push_tool
...
def main():
    ...
    robot, names, movable, body_path, grouping, pull_tool, push_tool = load_world()
    print('Objects:', names)
    saver = WorldSaver()

    problem = pddlstream_from_problem(robot, body_path, grouping, pull_tool=pull_tool, push_tool=push_tool, movable=movable, teleport=args.teleport)

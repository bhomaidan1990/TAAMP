...
def get_cfree_pose_pose_test(collisions=True, body_path={}, **kwargs):
    def test(b1, p1, b2, p2):
        print "Meiying::get_cfree_pose_pose_test"
        if not collisions or (b1 == b2):
            return True
        p1.assign()
        p2.assign()
        return not pairwise_collision(b1, b2, body_path=body_path, **kwargs) #, max_distance=0.001)
    return test


def get_cfree_obj_approach_pose_test(collisions=True, body_path={}):
    def test(b1, p1, g1, b2, p2):
        print "Meiying::get_cfree_obj_approach_pose_test"
        if not collisions or (b1 == b2):
            return True
        p2.assign()
        ...
        for obj_pose in interpolate_poses(grasp_pose, approach_pose):
            ...
            if pairwise_collision(b1, b2, body_path=body_path):
                return False
        return True
    return test

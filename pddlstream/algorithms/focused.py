...
from pddlstream.language.statistics import load_stream_statistics, \
    write_stream_statistics, compute_plan_effort, dump_total_statistics
...
def solve_abstract(problem, constraints=PlanConstraints(), stream_info={}, replan_actions=set(),
                  ...
                  visualize=False, verbose=True, is_meiying_condition=False, **search_kwargs):
    """
    ...
    #initial_complexity = 0 # TODO: Meiying, to delete
    
    #print("[Meiying::focused::solve_abstract] is_meiying_condition:", is_meiying_condition)
    ...
    #print("!!!!!!!!!!!!!!!!!!!!!!!!use_skeletons", use_skeletons) # True
    #print("!!!!!!!!!!!!!!!!!!!!!!!!eager_disabled", eager_disabled) # True
    #print("!!!!!!!!!!!!!!!!!!!!!!!!num_iterations", num_iterations) # 0
    #print("!!!!!!!!!!!!!!!!!!!!!!!!complexity_limit", complexity_limit) # 0
    ...
    eager_externals = list(filter(lambda e: e.info.eager, externals)) # eager is the predicate defined in streams files
    ...
    while (not store.is_terminated()) and (num_iterations < max_iterations) and (complexity_limit <= max_complexity):
        ...
        optimistic_solve_fn = get_optimistic_solve_fn(goal_exp, domain, negative,
                                                      ...
                                                      max_effort=max_effort, effort_weight=effort_weight, is_meiying_condition=is_meiying_condition, **search_kwargs)
        ...
        if (max_skeletons is None) or (len(skeleton_queue.skeletons) < max_skeletons):
            ...
            stream_plan, opt_plan, cost = iterative_plan_streams(evaluations, positive_externals,
                optimistic_solve_fn, complexity_limit, max_effort=max_effort, is_meiying_condition=is_meiying_condition)
            #print("[Meiying::focused::solve_abstract] 1. stream_plan:")
            #print(stream_plan)
        ...
        #print("[Meiying::focused::solve_abstract] 1.2 stream_plan:")
        #print(stream_plan)
        
        if stream_plan:
            reordered_stream_plan = []
            for stream in stream_plan:
                if "test" in stream.name and "feasible" in stream.name:
                    is_lazy_plan = False
                    for arg in stream.input_objects:
                        if "#" in str(arg):
                            is_lazy_plan = True
                            break
                    if not is_lazy_plan:
                        reordered_stream_plan.append(stream)
            for stream in stream_plan:
                if stream not in reordered_stream_plan:
                    reordered_stream_plan.append(stream)
            stream_plan = reordered_stream_plan
            #print("[Meiying::focused::solve_abstract] 1.5 stream_plan:")
            #print(stream_plan)
        ...
        if not use_skeletons: # do not use skeleton
...
def solve_adaptive(problem, max_skeletons=INF, search_sample_ratio=1, is_meiying_condition=False, **kwargs):
    """
    Solves a PDDLStream problem by first planning with optimistic stream outputs and then querying streams
    :param problem: a PDDLStream problem
    ...
    #assert search_sample_ratio > 0
        ...
        #print("[Meiying::focused::solve_adaptive] is_meiying_condition: ", is_meiying_condition)
    
    return solve_abstract(problem, max_skeletons=max_skeletons, search_sample_ratio=search_sample_ratio,
                          bind=None, max_failures=None, is_meiying_condition=is_meiying_condition, **kwargs)

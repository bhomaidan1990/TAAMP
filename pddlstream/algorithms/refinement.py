...
def optimistic_process_instance(instantiator, instance, verbose=False):
    for result in instance.next_optimistic():
        ...
        for fact in result.get_certified():
            evaluation_current = evaluation_from_fact(fact)
            instantite_result = instantiator.add_atom(evaluation_current, complexity)
            new_facts |= instantite_result
...
def get_optimistic_solve_fn(goal_exp, domain, negative, max_cost=INF, is_meiying_condition=False, **kwargs):
    # TODO: apply to hierarchical actions representations (will need to instantiate more actions)
    #print("[Meiying::refinement::get_optimistic_solve_fn] is_meiying_condition:", is_meiying_condition)
    def fn(evaluations, results, constraints):
        if constraints is None:
            #print("[Meiying::refinement::get_optimistic_solve_fn::fn] is_meiying_condition:", is_meiying_condition)
            return plan_streams(evaluations, goal_exp, domain, results, negative,
                                max_cost=max_cost, is_meiying_condition=is_meiying_condition, **kwargs)
...
def hierarchical_plan_streams(evaluations, externals, results, optimistic_solve_fn, complexity_limit,
                              depth, constraints, is_meiying_condition=False, **effort_args):
    
    #print("[Meiying::refinement::hierarchical_plan_streams] is_meiying_condition:", is_meiying_condition)
    ...
    return hierarchical_plan_streams(evaluations, externals, next_results, optimistic_solve_fn, complexity_limit,
                                     new_depth, next_constraints, is_meiying_condition=is_meiying_condition, **effort_args)

def iterative_plan_streams(all_evaluations, externals, optimistic_solve_fn, complexity_limit, is_meiying_condition=False, **effort_args):
    # Previously didn't have unique optimistic objects that could be constructed at arbitrary depths
    #print("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~externals")
    #print(externals)
    
    #print("[Meiying::refinement::iterative_plan_streams] is_meiying_condition:", is_meiying_condition)
    ...
    while num_iterations < 10:
        ...
        #print("[Meiying::refinement::iterative_plan_streams] results:")
        #print(results)
        opt_solution, final_depth = hierarchical_plan_streams(
            ...
            depth=0, constraints=None, is_meiying_condition=is_meiying_condition, **effort_args)
    ...
    return OptSolution(FAILED, FAILED, 0)

...
class StreamInstance(Instance):
    ...
    def __init__(self, stream, input_objects, fluent_facts, atpose_fluent_facts=[]):
        super(StreamInstance, self).__init__(stream, input_objects)
        self._generator = None
        self.fluent_facts = frozenset(fluent_facts)
        self.atpose_fluent_facts = [frozenset(fact) for fact in atpose_fluent_facts]
        ...
        #if self.external.name == "sample-pose":
            #print "[Meiying::StreamInstance::__init__] sample-pose, atpose_fluent_facts: {}".format(atpose_fluent_facts)
    ...
    def get_fluent_values(self):
        #if self.external.name == "sample-pose":
            #print "[Meiying::stream::get_fluent_values] "
            #print [Fact(get_prefix(f), values_from_objects(get_args(f))) for f in self.fluent_facts]
        return [Fact(get_prefix(f), values_from_objects(get_args(f))) for f in self.fluent_facts]

    def get_atpose_fluent_values(self):
        values = []
        for fact in self.atpose_fluent_facts:
            values.append([Fact(get_prefix(f), values_from_objects(get_args(f))) for f in fact])
        #if self.external.name == "sample-pose":
            #print "[Meiying::stream::get_atpose_fluent_values]"
            #print values   
        return values
    ...
    def _create_generator(self):
        if self._generator is None:
            ...
            elif len(self.atpose_fluent_facts) != 0:
                self._generator = self.external.gen_fn(*input_values, at_pose_fluents=self.get_atpose_fluent_values())
            else:
    ...
    def next_results(self, verbose=False):
        ...
        #print "currently running:", self.__repr__()
        new_values, new_facts = self._next_outputs()
        #print "get next output!!"
    ...
    def next_optimistic(self):
        ...
        for output_list in opt_gen.history:
            ...
            for output_values in output_list:
                ...
                #print("output_objects", output_objects)
    ...
    def remap_inputs(self, bindings):
        ...
        #    return self
        #print "Meiying::StreamInstance::remap_inputs for {} with bindings".format(self.__repr__())
        ...
        #if self.external.name == "sample-pose":
            #print "[Meiying::stream::StreamInstance {}::remap_inputs] before remap {}".format(self.__repr__(), self.atpose_fluent_facts)
        atpose_fluent_facts = []
        #print "self.fluent_facts"
        #print self.fluent_facts
        #print "self.atpose_fluent_facts"
        #print self.atpose_fluent_facts
        for fact in self.atpose_fluent_facts:
            #print "fact"
            #print fact
            atpose_fluent_facts.append([substitute_fact(f, bindings) for f in fact])
        #if self.external.name == "sample-pose":
            #print "[Meiying::stream::StreamInstance {}::remap_inputs] after remap {}".format(self.__repr__(), atpose_fluent_facts)
        new_instance = self.external.get_instance(input_objects, fluent_facts=fluent_facts, atpose_fluent_facts=atpose_fluent_facts)
...
class Stream(External):
    ...
    def is_function(self):
        return False
    def get_instance(self, input_objects, fluent_facts=frozenset(), atpose_fluent_facts=[]):
        #if self.name == "sample-pose":
            #print "[Meiying::Stream {}::get_instance] atpose_fluent_facts: {}".format(self.name, atpose_fluent_facts)
            #print "[Meiying::Stream {}::get_instance] fluent_facts: {}".format(self.name, fluent_facts)
        input_objects = tuple(input_objects)
        fluent_facts = frozenset(fluent_facts)
        atpose_fluent_facts = [frozenset(fact) for fact in atpose_fluent_facts]
        assert all(isinstance(obj, Object) or isinstance(obj, OptimisticObject) for obj in input_objects)
        key = (input_objects, fluent_facts, tuple(atpose_fluent_facts))
        if key not in self.instances:
            self.instances[key] = self._Instance(self, input_objects, fluent_facts, atpose_fluent_facts=atpose_fluent_facts)
